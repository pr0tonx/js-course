...
const mark = {
  ...
  height: 1.69,
  calcBMI: function() {
    mark.bmi = (this.mass / (this.height * this.height)).toFixed(2);
...
const john = {
  ...
  height: 1.95,
  calcBMI: function() {
    john.bmi = (this.mass / (this.height * this.height)).toFixed(2);
...
if(mark.bmi > john.bmi) {
  console.log(`${mark.fullName} BMI's (${mark.bmi}) is higher than John's (${john.bmi})`);
} else if (john.bmi > mark.bmi) {
  console.log(`${john.fullName} BMI's (${john.bmi}) is higher than John's (${mark.bmi})`);
} else {
  console.log(`Both of them have the same bmi ${john.bmi}`);
}
