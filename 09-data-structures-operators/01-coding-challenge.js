...
Suppose we get data from a web service about a certain game ('game' variable on next page). In this challenge we're
gonna work with that data.
...
function printGoals(...scorers) {
  let scorerPlayers = 'The scorers are: ';
  for (let i = 0; i < scorers.length; i++) {

    if(i !== scorers.length - 1) {
      scorerPlayers += `${scorers[i]}, `;
    } else {
      scorerPlayers += `${scorers[i]}. Total goals: ${i + 1}`
    }
  }
  return scorerPlayers;
}

const goalsScored = printGoals(...game.scored);
